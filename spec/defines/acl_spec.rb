...
describe 'squid::acl' do
  ...
  context "when parameters are set" do 
    ...
    it { should contain_concat_fragment('squid_acl_myacl').with_content(/^acl\s+myacl\s+urlregex\shttp:\/\/example.org\/$/) }
    it { should contain_concat_fragment('squid_acl_myacl').with_content(/^acl\s+myacl\s+urlregex\shttp:\/\/example.com\/$/) }
  end
end
