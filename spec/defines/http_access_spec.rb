...
describe 'squid::http_access' do
  ...
  context "when parameters are set" do 
    let(:params) do
      ...
      it { should contain_concat_fragment('squid_http_deny_this and that').with_target('/tmp/squid.conf') }
      it { should contain_concat_fragment('squid_http_deny_this and that').with_order('20-08-deny') }
      it { should contain_concat_fragment('squid_http_deny_this and that').with_content(/^http_access\s+deny\s+this and that$/) }
    end
  end
end
